...
class EditObatScreen extends StatefulWidget {
  ...
  const EditObatScreen({super.key, required this.obat});
...
class _EditObatScreenState extends State<EditObatScreen> {
  ...
  void initState() {
    ...
    _durasiController = TextEditingController(
      text: widget.obat.durasiHari.toString(),
    );
    _tanggalMulai = DateFormat('yyyy-MM-dd').parse(widget.obat.tanggalMulai);
    _frekuensi = widget.obat.frekuensi;
    _waktuMinum = List.from(widget.obat.waktuMinum);
  }
  ...
  Future<void> _simpan() async {
    ...
    final obat = widget.obat.copyWith(
    ...
    final success = await provider.editObat(obat);
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(provider.error ?? 'Gagal memperbarui obat')),
      );
    }
  }
